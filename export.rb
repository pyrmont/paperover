...
if ARGV.length == 1
    ...
    tags = (config["tags"] == nil) ? "" : config["tags"]
elsif ARGV.length == 7 || ARGV.length == 8
    username = ARGV[0]
    password = ARGV[1]
    rss_uri = ARGV[2]
    source_slug = ARGV[3]
    destination_id = ARGV[4]
    public_status = (ARGV[5] == "true") ? true : false
    unread_status = (ARGV[6] == "true") ? true : false
    tags = (ARGV.length == 8) ? ARGV[7] : ""
else
    cli = HighLine.new
    ...
    tags = cli.ask("Please enter a comma-separated list of tags (leave blank for no tags):")
end
